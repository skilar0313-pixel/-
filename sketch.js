...
let boxAnim = 0;            // 新增：動畫進度 0..1
let boxAnimSpeed = 0.06;    // 新增：動畫速度
...
function draw() {
    ...
    // 更新動畫進度（從下往上彈出）
    boxAnim = min(1, boxAnim + boxAnimSpeed);
    // 簡單的 ease-out-back，增加彈性效果
    function easeOutBack(t) {
        let c1 = 1.70158;
        let c3 = c1 + 1;
        return 1 + c3 * Math.pow(t - 1, 3) + c1 * Math.pow(t - 1, 2);
    }
    let eased = easeOutBack(boxAnim);

    // 算出方框中心 Y（從畫面下方到畫面中央）
    let startY = height + boxHeight / 2;
    let targetY = height / 2;
    let boxCenterY = lerp(startY, targetY, eased);
    ...
    stroke(0);
    strokeWeight(2);
    rect(width/2, boxCenterY, boxWidth, boxHeight , 12);
    ...
    let textTopY = boxCenterY - boxHeight/2 + offset;
    ...
    if (profileImg) {
        ...
        // 圖片 Y 放在方塊底部內側（使用 boxCenterY 計算）
        image(profileImg, width / 2 ,  boxCenterY + boxHeight * 0.18 );
        pop();
    }
    pop();
